...
<?php
	...
	if (!$isbn){
		echo "<h2>Manually insert or lookup item in database</h2><h3>Be sure to use number under barcode, not number labeled 'isbn', unless there is no barcode available. Usually the barcode's isbn and the number labeled 'isbn' differ slightly, which would cause the item to be duplicated in the database. i.e. bad.</h3><div class=\"cont\">isbn, ean, or plugid: <form action='" . $_SEVER['php_self'] . "' method='GET'><input type='text' name='q' id='q'><br/><input type='submit' value='Lookup'></form></div>";
	}
	else{
		...
		foreach($feed as $entry) {
			...
			echo "<h2>" . $titles . "</h2>" . "<h3>By " . $authors . "</h3>" . "<div class=\"cont\"><img src=\"" . $thumb . "\" class=\"img\" /> Published by: " . $publishers . "<br/><br/> Published on: " . $pubdate . "<br/><br/>" . $description;
			...
			echo "<input type='text' name='user_answer' /></form></div>";
		}
		...
		if ($totResults == 0)
		{
			...
			if (!$resp->faultCode())
			{
				...
				if ($data['message'] != "No database entry found.") {
					echo "<h2>UCC-13/EAN Located!</h2><h3>" . $data['description'] . "</h3><div class=\"cont\">";
